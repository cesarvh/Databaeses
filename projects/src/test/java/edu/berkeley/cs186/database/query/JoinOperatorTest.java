...
public class JoinOperatorTest {
  ...
  public void testDuplicateTableCreation() throws QueryPlanException, DatabaseException, IOException {
      File tempDir = tempFolder.newFolder("tableTest");
      Database d = new Database(tempDir.getAbsolutePath(), 4);
      Database.Transaction transaction = d.beginTransaction();


      QueryOperator s1 = new SequentialScanOperator(transaction,"leftTable");
      Iterator<Record> s1Iterator = s1.iterator();


      d = new Database(tempDir.getAbsolutePath(), 4);
      transaction = d.beginTransaction();
  ...
  @Test
  @Category(StudentTestP2.class)
  public void testSameTableDifferentOutputs() throws QueryPlanException, DatabaseException, IOException {
    File tempDir = tempFolder.newFolder("tableTest");
    Database d = new Database(tempDir.getAbsolutePath(), 4);
    Database.Transaction transaction = d.beginTransaction();
    ...
    QueryOperator s1 = new SequentialScanOperator(transaction,"leftTable");
    Iterator<Record> s1Iterator = s1.iterator();
    tempDir = tempFolder.newFolder("tableTest2");
    d = new Database(tempDir.getAbsolutePath(), 4);
    transaction = d.beginTransaction();
    ...
    QueryOperator s2 = new SequentialScanOperator(transaction,"leftTable");

    Iterator<Record> s2Iterator = s2.iterator();

    while (s1Iterator.hasNext() && s2Iterator.hasNext()) {
      assertNotEquals(s1Iterator.next(), s2Iterator.next());
    }
  ...
  public void testGraceHashDifferentThanBNLJ() throws QueryPlanException, DatabaseException, IOException {
    File tempDir = tempFolder.newFolder("joinTest");
    Database d = new Database(tempDir.getAbsolutePath(), 4);
    Database.Transaction transaction = d.beginTransaction();
    Record r1 = TestUtils.createRecordWithAllTypesWithValue(1);
    ...
    assertTrue(count == 82944);
    assertEquals(12, joinOperator.getIOCost());

  }

  @Test
  @Category(StudentTestP2.class)
  public void testGraceHashHasSameIOCostAndDiffFromOtherJoins() throws IOException, DatabaseException, QueryPlanException {
    File tempDir = tempFolder.newFolder("joinTest");
    Database d = new Database(tempDir.getAbsolutePath(), 4);
    Database.Transaction transaction = d.beginTransaction();
    Record r1 = TestUtils.createRecordWithAllTypesWithValue(1);
    ...
    QueryOperator s1 = new SequentialScanOperator(transaction, "leftTable");
    QueryOperator s2 = new SequentialScanOperator(transaction, "rightTable");
    QueryOperator graceHashOperator1 = new GraceHashOperator(s1, s2, "int", "int", transaction);
    QueryOperator graceHashOperator2 = new GraceHashOperator(s2, s1, "int", "int", transaction);
    QueryOperator pnljOperator = new PNLJOperator(s1, s2, "int", "int", transaction);
    QueryOperator bnljOperator = new BNLJOperator(s1, s2, "int", "int", transaction);
    QueryOperator snljOperator = new SNLJOperator(s1, s2, "int", "int", transaction);

    assertEquals(graceHashOperator1.getIOCost(), graceHashOperator2.getIOCost());
    assertNotEquals(graceHashOperator1.getIOCost(), snljOperator.getIOCost());
    assertNotEquals(graceHashOperator1.getIOCost(), bnljOperator.getIOCost());
    assertNotEquals(graceHashOperator1.getIOCost(), pnljOperator.getIOCost());
    assertTrue(graceHashOperator1.getIOCost() < pnljOperator.getIOCost());
  }
